...
2. **Fixed evaluator** — freeze the suite, machine, source, build, model, and
   comparison policy before any candidate patch is applied. The evaluator does
   not move mid-campaign.
...
5. **Fresh confirmation** — if screening looks promising, run a new baseline
   and a new candidate (not a reuse of the screening runs) against the same
   frozen evaluator.
6. **Retain or reject** — keep the finding, including correctness failures,
   regressions, mixed results, and inconclusive comparisons. Then stop or
   hand the next hypothesis to a **new** campaign.
...
- Calibration is two distinct no-op baseline suite runs. The campaign fails
  closed unless they compare `inconclusive` and pass the campaign-only
  cross-arm greedy token-parity gate.
- Screening and confirmation each use whole-suite AB then BA order (four
  suite runs per phase). That order balances arms; it is not temporally
  paired sampling and is not a campaign-level confidence interval.
...
- Failed, interrupted, and budget-exhausted attempts stay spent. Resume does
  not replay a completed phase or confirm twice.
...
the finite remainder. `inspect` prints canonical JSON state; there is no
separate report command. State is persisted under the resolved StrixLab home
as `campaigns/<id>/state.json` with frozen input copies beside it. Exploration
versus confirmation is phase-internal — there is no extra flag that turns a
screen into a confirmation.
...
| `candidates[].id` | Dash identifier, unique in the plan |
| `candidates[].patches` | Nonempty ordered list of relative patch paths |
| `max_candidates` | Integer `>=` the candidate list length; a shorter cap is rejected |
| `max_suite_runs` | Integer `>= 0`, covering calibration, screening, confirmation, and partial or failed phases. Zero is allowed and stops before source preparation |
...
runs plus four screening runs plus four confirmation runs. Do not claim
statistical confidence from that budget. Core owns any schema-valid example
plan under `configs/campaigns/`; this document will link that fixture when it
...
1. **Core controller** — freeze-only `campaign create`, bounded `campaign
   resume`, JSON `campaign inspect`. Durable phase reservation before side
   effects; interrupted or unknown outcomes stay spent; next campaign is a
   new plan.
