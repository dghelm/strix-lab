...
@unittest.skipUnless(
    os.getuid() == 1000 and shutil.which("bwrap"), "requires local uid 1000 and Bubblewrap"
)
    ...
    def run_probe(self, defect=None):
        with tempfile.TemporaryDirectory(prefix="strix-preflight-test-") as temporary:
            ...
            for name in ("sdk", "input", "native", "work", "empty"):
                (root / name).mkdir()
            shutil.copyfile(
                Path(__file__).with_name("preflight_exec.py"), root / "input/preflight_exec.py"
            )
            host_ns = {
                name: os.readlink(f"/proc/self/ns/{name}")
                for name in ("user", "mnt", "pid", "ipc", "net")
            }
