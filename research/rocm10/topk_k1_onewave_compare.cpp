...
void launch(Resources& r, bool candidate, int rows, int columns) {
    if (candidate) {
        check(strixlab_topk_k1_onewave_hip(r.src, r.candidate, rows, columns, r.stream), "onewave-launch");
    } else {
        check(strixlab_topk_k1_small_hip(r.src, r.baseline, rows, columns, r.stream), "baseline-launch");
    }
}
...
void run(Resources& r, json& report) {
    ...
    for (const auto& in : inputs) {
        ...
        report["cases"].push_back({{"rows", rows}, {"columns", columns}, {"k", 1},
            {"baseline", "small"}, {"candidate", "onewave"},
...
int main() {
    Resources resources;
    json report = {{"schema_version", 1}, {"fixture", "rocm10-topk-k1-onewave-compare-v1"},
        ...
        {"comparison_order", "six shapes; small baseline paired with onewave candidate"},
        {"baseline_entry_point", "strixlab_topk_k1_small_hip"},
        {"candidate_entry_points", {{"onewave", "strixlab_topk_k1_onewave_hip"}}},
    ...
    try {
        ...
        success = false;
        std::cout << "{\"fixture\":\"rocm10-topk-k1-onewave-compare-v1\",\"success\":false,"
