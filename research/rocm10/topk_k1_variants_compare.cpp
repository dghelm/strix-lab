...
constexpr std::size_t allocated_bytes = capacity * 4 + 2 * 64 * 4;
...
struct Resources {
    float* src = nullptr;
    int *baseline = nullptr, *candidate = nullptr;
...
enum class Variant { small, wave };
const char* variant_name(Variant variant) {
    return variant == Variant::small ? "small" : "wave";
}
void launch(Resources& r, bool candidate, Variant variant, int rows, int columns) {
    if (!candidate) {
        check(strixlab_topk_k1_hip(r.src, r.baseline, rows, columns, r.stream), "baseline-launch");
    } else if (variant == Variant::small) {
        check(strixlab_topk_k1_small_hip(r.src, r.candidate, rows, columns, r.stream), "small-launch");
    } else {
        check(strixlab_topk_k1_wave_hip(r.src, r.candidate, rows, columns, r.stream), "wave-launch");
    }
}
void batch(Resources& r, bool candidate, Variant variant, int rows, int columns) {
    for (int i = 0; i < launches; ++i) launch(r, candidate, variant, rows, columns);
}
double measure(Resources& r, bool candidate, Variant variant, int rows, int columns) {
    check(hipEventRecord(r.start, r.stream), "record-start");
    batch(r, candidate, variant, rows, columns);
...
void run(Resources& r, json& report) {
    ...
    // Each candidate gets its own fresh paired baseline measurements on identical inputs.
    for (Variant variant : {Variant::small, Variant::wave}) {
        for (const auto& in : inputs) {
            const int rows = int(in.shape.rows), columns = int(in.shape.columns);
            report["cases"].push_back({{"rows", rows}, {"columns", columns}, {"k", 1},
                {"baseline", "original-k1"}, {"candidate", variant_name(variant)},
            ...
            launch(r, false, variant, rows, columns);
            launch(r, true, variant, rows, columns);
            check(hipStreamSynchronize(r.stream), "correctness-sync");
            validate(r, in, false);
            validate(r, in, true);
            result["correctness"] = true;
            for (int i = 0; i < warmups; ++i) {
                batch(r, false, variant, rows, columns);
                batch(r, true, variant, rows, columns);
            }
            ...
            for (int i = 0; i < samples; ++i) {
                for (bool is_candidate : {bool(i % 2), !bool(i % 2)}) {
                    double us = measure(r, is_candidate, variant, rows, columns);
...
int main() {
    Resources resources;
    json report = {{"schema_version", 1}, {"fixture", "rocm10-topk-k1-variants-compare-v1"},
        ...
        {"comparison_order", "small then wave; six shapes per candidate; baseline measured separately for each candidate"},
        {"baseline_entry_point", "strixlab_topk_k1_hip"},
        {"candidate_entry_points", {{"small", "strixlab_topk_k1_small_hip"}, {"wave", "strixlab_topk_k1_wave_hip"}}},
        {"pair_order", "even baseline,candidate; odd candidate,baseline; zero-based"},
        {"timing_scope", "HIP event batch elapsed / 100; all provider stream work including possible host enqueue gaps; excludes input transfer, validation, allocation, event creation"},
    ...
    try {
        ...
        success = false;
        std::cout << "{\"fixture\":\"rocm10-topk-k1-variants-compare-v1\",\"success\":false,"
